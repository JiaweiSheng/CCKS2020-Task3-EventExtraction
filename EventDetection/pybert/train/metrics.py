...
class AccuracyThresh(Metric):
    ...
    def __init__(self, thresh=0.5, normalizate=True):
...
class F1Score(Metric):
    ...
    def __call__(self, logits, target):
        ...
        elif not self.normalizate and self.task_type == 'binary':
            y_prob = logits.data.cpu().numpy()
        else:
            y_prob = logits.cpu().detach().numpy()

        if self.task_type == 'binary':
            if not self.search_thresh:
                self.y_pred = (y_prob > self.thresh).astype(int)
                self.value()
            else:
                thresh, f1 = self.thresh_search(y_prob=y_prob)
                print(f"Best thresh: {thresh:.4f} - F1-{self.average}: {f1:.4f}")
                self.thresh = thresh
                self.y_pred = (y_prob > self.thresh).astype(int)
...
class MultiLabelReportAucF1(Metric):
    '''
    multi label report
    provided by Jiawei Sheng 2020/08/18
    '''

    def __init__(self, id2label=None, thresh=0.5, normalizate=True):
        super(MultiLabelReportAucF1).__init__()
    ...
    def __call__(self, logits, target):
        if self.normalizate:
            ...
            self.y_prob = logits.data.cpu().detach().numpy()
        self.y_true = target.cpu().numpy()
    ...
    def value(self):
        ...
        for i, label in self.id2label.items():
            if 1 not in self.y_true[:, i]:
                self.y_true[0, i] = 1
            if 0 not in self.y_true[:, i]:
                self.y_true[0, i] = 0
            auc = roc_auc_score(y_score=self.y_prob[:, i], y_true=self.y_true[:, i])
            y_pred = (self.y_prob[:, i] > self.thresh).astype(int)
            f1 = f1_score(y_true=self.y_true[:, i], y_pred=y_pred, average='binary')
            print(f"label:{label} - auc: {auc:.4f} - f1: {f1: .4f}")

    def name(self):
        return "multilabel_report: auc and f1-score"
